...
class ActionServer():
    ...
    def __init__(self):
        self.a_server = actionlib.SimpleActionServer(
            "move_initial", ActionAction, execute_cb=self.execute_cb, auto_start=False)
        self.a_server.start()
        ...
        try:
            while not rospy.is_shutdown():
                ...
                if X == j1 and Y == j2 and Z == j3:
                    ...
                    stop_program()
                    result.status = "MOVED TO INITAL LOCATION"
                    print "MOVED TO INITAL LOCATION"
...
if __name__ == "__main__":
    rospy.init_node("move_initial_action_server")
    s = ActionServer()
    rospy.spin()
